...
// Fondo interactivo de fluidos — puerto adaptado del WebGL Fluid Simulation
// de Pavel Dobryakov (MIT License): https://github.com/PavelDoGreat/WebGL-Fluid-Simulation
//
// Es una simulación real de dinámica de fluidos (Navier-Stokes: curl → vorticidad
// → divergencia → presión (Jacobi) → gradiente → advección) corriendo en WebGL2,
// no partículas ni puntos: un campo de "tinte" continuo que fluye de verdad.
//
// Cambios respecto al original (que era un demo standalone en <script>):
// - Encapsulado en una función que recibe el canvas y devuelve un cleanup().
// - Sin dat.GUI, sin Google Analytics, sin popup promocional de apps móviles.
// - El puntero reacciona al simple movimiento del cursor (no requiere click):
//   se escucha en window (el canvas tiene pointer-events:none) y se trata como
//   "presionado" permanentemente para que el agua siga al cursor al pasar por
//   encima, en vez de exigir arrastrar con el botón apretado.
// - Colores del splat sesgados a la gama cian/azul de la marca en vez de todo
//   el arcoíris aleatorio.

const BRAND_HUE = 0.53; // ~cian de la marca (#00d2ff)
...
export function initFluidSimulation(canvas: HTMLCanvasElement): (() => void) | null {
  ...
  function compileShader(type: number, source: string, keywords: string[] | null = null) {
    const fullSource = addKeywords(source, keywords);
  ...
  function generateColor() {
    // rango angosto y bien saturado dentro de la familia cian/azul, con brillo
    // moderado — evita el blanco/washed-out que al hacer bloom se lee como
    // destello cálido (fuego) en vez de agua.
    const hue = wrap(BRAND_HUE + (Math.random() - 0.5) * 0.05, 0, 1);
    const c = HSVtoRGB(hue, 0.95, 0.7 + Math.random() * 0.15);
    c.r *= 0.11;
    c.g *= 0.11;
    c.b *= 0.11;
    return c;
  ...
  // El fluido sigue al cursor con solo pasar por encima (no requiere click):
  // el canvas tiene pointer-events:none, así que se escucha en window y el
  // puntero queda "activo" apenas se detecta el primer movimiento.
  ...
  function handlePointerMove(clientX: number, clientY: number) {
    const { x, y } = getCanvasRelativePos(clientX, clientY);
