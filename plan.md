...
| 8 — Migración a SSR | HECHO | Claude Code | 2026-08-13 | Migrado de SPA pura (CSR) a Server-Side Rendering con React Router v7 Framework Mode + `@vercel/react-router`. Motivo: Lighthouse en 3G real mostraba hasta 38s hasta interactividad — el HTML llegaba vacío y todo se armaba recién tras cargar/ejecutar el JS. Se agregaron `root.tsx`/`routes.ts`/`entry.client.tsx`/`entry.server.tsx` (reemplazan `index.html`/`App.tsx`/`main.tsx`), se movieron los fetches de Supabase de Nosotros/Proyectos/Productos a `loader()` (SSR + revalidación en navegación), y se corrigieron 4 puntos que leían `localStorage`/`matchMedia` de forma insegura para SSR (tema, idioma, splash del logo, navbar) — el patrón correcto no es solo evitar el crash con `typeof window`, sino que el primer render del cliente tiene que coincidir exactamente con el HTML del servidor (si no, error de hidratación); la corrección real ocurre recién en un `useEffect` post-montaje. Verificado: build limpio, las 8 rutas SSR sin errores de consola, cero mismatches de hidratación, batería de pruebas Playwright (6 páginas × 5 tamaños de pantalla) sin problemas, CLS bajó de ~0.4 a ~0.03-0.05. Nota: se detectó que el hosting real es Vercel (no Netlify como decía este documento) — se borró `public/_redirects` por quedar obsoleto. Los componentes 3D (Three.js) se dejaron sin cambios a propósito (client-only, lazy) por pedido explícito del usuario. Pendiente: confirmar métricas finales de Lighthouse contra el deploy real de Vercel (las pruebas locales contra el servidor de desarrollo no son representativas de producción). |
